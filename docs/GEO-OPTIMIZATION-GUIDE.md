...
## 0-G. 구글 공식 입장(AI 기능 최적화 가이드) — 과잉 최적화 금지

출처: [SEO 기본 가이드](https://developers.google.com/search/docs/fundamentals/seo-starter-guide?hl=ko) ·
[AI 기능 최적화 가이드](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide?hl=ko) (2026-08-02 확인)

AI 개요(AI Overviews)·AI 모드에 노출되는 조건은 **별도 기법이 아니라 일반 검색 색인**이다.
구글이 명시적으로 "효과 없다"고 한 것에 시간을 쓰지 않는다.

| 구글 공식 입장 | 우리 적용 |
|---|---|
| 생성형 AI 검색에 **구조화 데이터는 필수가 아님**(단, 리치 결과용으로 계속 권장) | JSON-LD는 유지하되 *리치 결과·엔티티 명확성* 목적임을 인지 |
| **llms.txt는 구글 검색이 무시**한다 | `/llms.txt`는 유지하되 **구글 대비 수단이 아님** — ChatGPT·Perplexity 등 타 엔진용 보조 자산으로만 취급 |
| AI용 **콘텐츠 청킹 불필요** — 한 페이지의 여러 주제를 이해함 | 인용 가능한 문장 구조는 가독성 목적이지, 잘게 쪼갠 페이지 양산 금지 |
| 팬아웃 쿼리별 페이지 양산은 **확장된 콘텐츠 악용 스팸 정책 위반** | 키워드 변형용 얇은 페이지 생성 금지 |
| "AEO/GEO"라 불러도 **여전히 SEO** | 본 가이드의 GEO 항목은 SEO와 분리된 별도 작업이 아님 |
| 노출 전제: **색인 + 스니펫 표시 가능**해야 함 | `nosnippet`·과도한 `max-snippet` 금지, 비공개 페이지만 noindex |

> 결론: **AI 전용 트릭이 아니라 "색인 가능 + 고유하고 유용한 콘텐츠 + 스니펫 허용"** 이 GEO의 실제 조건이다.
