...
와 같이 **결과물을 만드는 업무 단위**로 Agent에게 요청할 수 있는 환경을 만드는 것입니다.
...
이 연결을 자동으로 열어두지 않고 단계로 분리한 것이 핵심입니다. 웹에 있는 Agent가 사용자의 실제 업무환경에서 실행할 수 있는 Agent로 확장되는 지점이라, 무엇을 열어줄지는 명시적으로 정해야 합니다.
...
XGEN은 Agent를 단순한 LLM 호출 단위가 아니라 **LLM + Memory + Workspace + Tool + Execution Environment**가 결합된 업무 수행 단위로 확장하는 방향을 지향합니다. Agent가 작업한 파일과 업무 맥락을 이어받아 하나의 업무를 연속적으로 수행하려면 Workspace가 필요합니다.
...
PC에 존재하는 파일이나 애플리케이션을 다뤄야 하는 업무는 로컬 실행환경을 활용하는 것이 자연스럽습니다.
...
보기 좋으라고 넣은 장식이 아닙니다. Agent를 **찾아가는 대상이 아니라 옆에 있는 동료로** 두려는 것입니다. 서버에서 관리되는 Agent가 사용자의 업무환경에 실제로 자리 잡았다는 것을, 화면에서 가장 단순하게 보여주는 방식이기도 합니다.
