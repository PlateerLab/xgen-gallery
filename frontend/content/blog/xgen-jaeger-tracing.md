...
세 번의 선택을 관통한 질문은 결국 하나였습니다. **계측을 어디에 둘 것인가.** 프록시(Envoy), 커널(eBPF), 애플리케이션 프로세스(OTel) — 같은 요청을 보더라도 어디서 보느냐에 따라 남는 기록이 달랐습니다.

프록시 레벨은 요청이 지나간 경로는 보여줬지만 그 요청이 무엇을 했는지는 보여주지 못했습니다. Trace Context를 애플리케이션으로 넘기지 못해 서비스 경계마다 이야기가 끊긴 것도 같은 이유였습니다. 관측의 위치가 곧 관측의 해상도였던 셈입니다.

또 하나는 기술 선택이 문서가 아니라 운영 환경에서 결정된다는 점이었습니다. eBPF는 `BEYLA_BPF_TRACK_REQUEST_HEADERS`라는 정확한 답을 갖고 있었지만, 서버에 올라가 있는 RHEL 4.18 커널 하나에 막혔습니다. 가장 앞선 방식이 무엇인가보다, 지금 이 서버에서 무엇이 도는가가 더 자주 결론을 냅니다.

글머리에 밝혔듯 이번 도입은 모범적인 형태와는 거리가 있을 수 있습니다. 다만 목표는 처음부터 분명했습니다 — 병목 지점 분석에 Trace를 실제로 활용해보고, 신규 프로젝트에서 더 실무적으로 쓸 수 있는 아키텍처로 발전시키는 것. 애플리케이션 레벨 Span이 보이고 서비스 경계를 넘어 Trace가 이어지는 지금이 그 출발점입니다.
