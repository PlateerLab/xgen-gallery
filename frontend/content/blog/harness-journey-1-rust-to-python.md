...
비슷한 문제가 L사 상품 심사 Agent QA에서도 나타났습니다. 네 단계의 상품 심사 Agent QA 실행 기록을 65개 판정 규칙으로 재생하면 결과가 잘못됐다는 사실은 찾을 수 있었지만, 운영 실행 중에 같은 규칙을 일관되게 적용하기는 어려웠습니다. 기존 실행기에 판정 미들웨어를 끼우는 방식도 검토했지만, 제품의 노드 실행 코드와 도메인 규칙이 한 계층에 결합돼 변경 범위가 너무 커졌습니다. 생성 모델 바깥에서 검증과 재시도를 소유하는 별도 실행 계층이 필요했습니다.
