...
interface CaCopy {
    ...
    specs: [string, string][];
    outcomes: { title: string; before: string; after: string; value: string }[];
    trust: [string, string][];
    pillars: [string, string][];
    capabilities: { title: string; en: string; desc: string }[];
...
const SPEC_ICONS: LucideIcon[] = [
    Code2, GitBranch, SplitSquareVertical, Search, FileCheck2, Workflow, Layers, ShieldCheck,
];
const OUTCOME_ICONS: LucideIcon[] = [Boxes, ShieldCheck, FileCheck2, Workflow];
const TRUST_ICONS: LucideIcon[] = [ShieldCheck, Boxes, GitBranch, Layers];
const PILLAR_ICONS: LucideIcon[] = [Boxes, ShieldCheck, Layers];
const CAPABILITY_ICONS: LucideIcon[] = [
    GitBranch, Database, SplitSquareVertical, FileCheck2, Code2, Search,
];
const INTEGRATION_ICONS: LucideIcon[] = [GitBranch, ShieldCheck, Workflow];
const USE_CASE_ICONS: LucideIcon[] = [Workflow, Search, Boxes, FileCheck2];

const COPY: Record<Locale, CaCopy> = {
    ko: {
        sections: [
        ...
        specs: [
            ["개발환경 (IDE)", "VS Code 확장 — 인라인 자동완성·컨텍스트 메뉴·Diagnostics를 에디터에 통합"],
            ["저장소 연동", "GitLab 코드 저장소 연계 · 실시간 증분 인덱싱으로 최신 코드를 동기화"],
            ["코드 이해", "AST 기반 구조화 — 파일 단위가 아닌 호출·의존 관계로 코드베이스를 학습"],
            ["하이브리드 검색", "키워드(BM25) + 의미 벡터 검색을 결합하고 AI 재정렬(Re-rank)로 정확도 향상"],
            ["문서 파싱", "xlsx·docx·ppt·PDF·HTML 등 다양한 산출물을 통합 파싱해 컨텍스트로 활용"],
            ["도구·데이터 연동", "MCP(Model Context Protocol) 표준으로 이기종 시스템·DB를 안전하게 연계"],
            ["모델", "온프레미스 self-host · 임베딩·리랭커 · 원하는 LLM을 선택·교체(모델 중립)"],
            ["보안", "망분리·폐쇄망 · RBAC 접근통제 · 감사로그 · 시큐어코딩 기반 운영"],
        ],
        outcomes: [
            {
        ...
        trust: [
            ["온프레미스 설치", "소스·데이터 사내 보관"],
            ["폐쇄망 대응", "Air-gap 환경 배포"],
            ["GitLab 접근통제", "계정·권한 기반"],
            ["모델 중립", "원하는 LLM 선택·교체"],
        ],
        pillars: [
            ["프로젝트 전체를 이해", "코드뿐 아니라 API·DB 스키마·정책 문서까지 학습해, 무엇을 어디에 만들지 코드 수준으로 안내합니다."],
            ["온프레미스 보안", "사내 인프라에 설치돼 소스와 데이터가 외부로 나가지 않습니다. GitLab 계정으로 접근을 통제합니다."],
            ["모델 중립", "관리형 AI 게이트웨이로 원하는 LLM을 선택·교체하며, AI 생태계 변화에 지속적으로 대응합니다."],
        ],
        capabilities: [
            { title: "저장소 기반 응답", en: "Repository-Grounded", desc: "GitLab 저장소를 선택하면 해당 코드베이스에 근거해 답합니다. 실제 컴포넌트·파일 구조를 참조해 재사용 방법과 구현 절차를 제시합니다." },
            { title: "스키마 인지", en: "Schema-Aware", desc: "DB 스키마와 테이블 간 관계(관계·카디널리티·설명)를 학습해, 특정 업무에 필요한 테이블·API를 정확히 짚어 줍니다." },
            { title: "질문 분해", en: "Question Decomposition", desc: "질문을 API·CODE·TABLE 단위로 분해한 뒤 종합해, 단편적 답변 대신 더 풍부하고 정밀한 답변을 생성합니다." },
            { title: "코드 리뷰·개선 제안", en: "Code Review", desc: "기존 코드의 문제점을 짚고, 인증·오류 처리·성능 등 코드 수준의 구체적인 개선안을 예시 코드와 함께 제안합니다." },
            { title: "신규 기능 설계 지원", en: "Feature Design", desc: "재사용할 컴포넌트·패턴을 찾아 구현 절차와 예시 코드를 제시해, 기획 아이디어를 실제 구현으로 잇습니다." },
            { title: "엔드포인트 탐색", en: "API Discovery", desc: "방대한 코드베이스에서 필요한 API 엔드포인트와 관련 컨트롤러를 찾아, 정확한 경로와 사용법을 안내합니다." },
        ],
        steps: [
            { title: "저장소 인덱싱", desc: "GitLab 저장소를 전체·증분 동기화로 인덱싱하고, 코드·API·DB 스키마·산출물을 벡터로 학습합니다." },
            { title: "컨텍스트 검색 (RAG)", desc: "질문을 분해해 관련 코드·스키마·문서를 검색하고, 근거로 삼아 접지(grounding)합니다." },
            { title: "MCP 파이프라인", desc: "Model Context Protocol로 도구와 컨텍스트를 표준 방식으로 연결해, 필요한 정보를 안전하게 가져옵니다." },
            { title: "코드 수준 응답", desc: "검색한 근거에 접지해, 프로젝트 맥락에 맞는 코드 수준의 답변을 생성합니다." },
        ],
        integrations: [
            ["GitLab 연동", "GitLab 계정으로 로그인하고 저장소를 연동합니다. GitLab Duo 하이브리드 구성으로 언어 커버리지를 넓힙니다."],
            ["온프레미스 배포", "사내 GPU 서버에 설치하고, 관리형 AI 게이트웨이로 폐쇄망 환경까지 대응합니다."],
            ["산출물 RAG 연계", "정책서·설계 문서 등 도메인 산출물을 RAG로 추가해, 코드 밖의 맥락까지 답변에 반영합니다."],
        ],
        comparison: [
        ...
        useCases: [
            { title: "신규 기능 구현 가이드", points: ["재사용 가능한 컴포넌트·패턴을 찾아 제시", "구현 절차와 예시 코드까지 함께 안내", "관련 API·테이블을 짚어 설계로 연결"] },
            { title: "API · 스키마 탐색", points: ["필요한 엔드포인트와 컨트롤러 위치 파악", "DB 테이블·관계·카디널리티 확인", "업무에 맞는 API·테이블을 정확히 지목"] },
            { title: "레거시 이해 · 온보딩", points: ["대규모 코드베이스 구조를 코드 수준으로 설명", "문서화되지 않은 로직도 근거와 함께 파악", "신규 인력의 온보딩 기간 단축"] },
            { title: "코드 리뷰 · 개선", points: ["인증·오류 처리·성능 등 문제점 진단", "예시 코드가 포함된 구체적 개선안", "규칙·보안 위반 사전 점검"] },
        ],
        faq: [
        ...
        capabilitiesLead: "사내 코드·API·스키마를 학습해 프로젝트 맥락에서 정확하게 답하는 기능들입니다.",
        howTitle: "작동 원리",
        howLead: "저장소를 인덱싱해 근거를 만들고, 질문에 맞는 컨텍스트를 검색해 코드 수준의 답을 생성합니다.",
        archNote: "게이트웨이 기반 인증 격리와 인덱싱·RAG·MCP로 구성된 전체 구조는 아키텍처 문서에서 자세히 확인할 수 있습니다.",
        archCta: "코드 어시스턴트 아키텍처 자세히 보기",
        comparisonTitle: "범용 코드 어시스턴트와 무엇이 다른가",
        comparisonGeneric: "범용 코드 어시스턴트",
        integrationsTitle: "연동과 배포",
        specsTitle: "기술 사양 · 연동",
        ...
        ctaTitle: "우리 코드베이스에 맞춰 검토해 보세요",
        ctaLead: "사내 저장소와 개발 환경에 맞는 도입 방안을 함께 설계해 드립니다.",
        ctaPrimary: "도입 문의",
        ctaSecondary: "보안·도입 요건 상담",
    },
    en: {
        sections: [
        ...
        specs: [
            ["Development environment (IDE)", "A VS Code extension — inline completion, context menu, and diagnostics inside the editor"],
            ["Repository integration", "Connected to GitLab repositories, with real-time incremental indexing keeping the code current"],
            ["Code comprehension", "AST-based structuring — the codebase is learned through call and dependency relationships, not file by file"],
            ["Hybrid search", "Keyword (BM25) combined with semantic vector search, then AI reranking for accuracy"],
            ["Document parsing", "Unified parsing of xlsx, docx, ppt, PDF, HTML, and other artifacts, used as context"],
            ["Tool and data integration", "Heterogeneous systems and databases connected safely through the MCP standard"],
            ["Models", "On-premise self-hosting, embedding and reranker models, and your choice of LLM — swappable, model-agnostic"],
            ["Security", "Network separation and air-gap, RBAC access control, audit logs, and secure-coding practice"],
        ],
        outcomes: [
            {
                title: "New developers onboard faster",
                before: "Weeks to months for a new hire to understand the legacy code",
                after: "Ask the project code directly and grasp classes, APIs, database, and call flow immediately",
                value: "Shorter ramp-up and higher developer productivity",
            },
            {
                title: "Source code never leaves the building",
                before: "Using an external AI service means shipping source code out, with the security concerns that follow",
                after: "Code and data stay internal in an on-premise or air-gapped environment",
                value: "Meets security policy in finance, public sector, and large enterprises",
            },
            {
                title: "Answers that fit your project",
                before: "Generic code explanations from a general-purpose AI",
                after: "Answers grounded in your project code, citing the actual components, APIs, tables, and source files",
                value: "Accurate answers with code evidence you can check",
            },
            {
                title: "Maintenance productivity rises sharply",
                before: "Code exploration and analysis that depends on one person's experience",
                after: "One question surfaces the related code, call relationships, and blast radius",
                value: "Faster incident response and change-impact analysis",
            },
        ],
        trust: [
            ["On-premise install", "Source and data stay in-house"],
            ["Air-gap ready", "Deploys into network-separated environments"],
            ["GitLab access control", "Account and permission based"],
            ["Model-agnostic", "Pick and swap the LLM you want"],
        ],
        pillars: [
            ["Understands the whole project", "It learns not just the code but APIs, database schemas, and policy documents, so it can tell you what to build and where, at code level."],
            ["On-premise security", "Installed on your own infrastructure, so source and data never leave. Access is controlled by GitLab account."],
            ["Model-agnostic", "A managed AI gateway lets you pick and swap the LLM, so you keep pace as the AI ecosystem shifts."],
        ],
        capabilities: [
            { title: "Repository-grounded answers", en: "Repository-Grounded", desc: "Select a GitLab repository and answers are grounded in that codebase. It references the real components and file structure to show how to reuse them and how to implement." },
            { title: "Schema-aware", en: "Schema-Aware", desc: "It learns the database schema and the relationships between tables — cardinality and descriptions included — so it points to exactly the tables and APIs a task needs." },
            { title: "Question decomposition", en: "Question Decomposition", desc: "A question is broken into API, CODE, and TABLE parts, then recombined, producing a fuller and more precise answer than a single-shot response." },
            { title: "Code review and improvement", en: "Code Review", desc: "It identifies problems in existing code and proposes concrete fixes at code level — authentication, error handling, performance — with example code." },
            { title: "Feature design support", en: "Feature Design", desc: "It finds the components and patterns worth reusing and lays out the implementation steps with example code, connecting an idea to a real build." },
            { title: "Endpoint discovery", en: "API Discovery", desc: "Across a large codebase, it finds the API endpoint and related controllers you need, with the exact path and how to use them." },
        ],
        steps: [
            { title: "Repository indexing", desc: "GitLab repositories are indexed through full and incremental sync, learning code, APIs, database schemas, and artifacts as vectors." },
            { title: "Context retrieval (RAG)", desc: "The question is decomposed, related code, schemas, and documents are retrieved, and the answer is grounded in them." },
            { title: "MCP pipeline", desc: "The Model Context Protocol connects tools and context in a standard way, pulling in the information needed safely." },
            { title: "Code-level answer", desc: "Grounded in the retrieved evidence, it generates an answer at code level that fits your project's context." },
        ],
        integrations: [
            ["GitLab integration", "Sign in with a GitLab account and connect your repositories. A GitLab Duo hybrid setup widens language coverage."],
            ["On-premise deployment", "Installed on your own GPU servers, with a managed AI gateway covering air-gapped environments."],
            ["Artifact RAG", "Domain artifacts such as policy and design documents are added through RAG, so context beyond the code reaches the answer."],
        ],
        comparison: [
        ...
        useCases: [
            { title: "Guidance for building a new feature", points: ["Finds and presents reusable components and patterns", "Lays out the implementation steps with example code", "Points to the related APIs and tables, connecting into the design"] },
            { title: "API and schema discovery", points: ["Locates the endpoints and controllers you need", "Confirms database tables, relationships, and cardinality", "Points precisely to the APIs and tables the task requires"] },
            { title: "Legacy comprehension and onboarding", points: ["Explains a large codebase's structure at code level", "Surfaces undocumented logic together with its evidence", "Shortens the onboarding period for new hires"] },
            { title: "Code review and improvement", points: ["Diagnoses problems in authentication, error handling, and performance", "Concrete improvements that include example code", "Pre-checks for rule and security violations"] },
        ],
        faq: [
        ...
        ldFeatures: [
            ...
            "Question decomposition (API / CODE / TABLE)",
        ...
        heroChip: "Enterprise · on-premise coding assistant",
        overviewTitle: "AI shouldn't just generate code — it should understand where that code lives",
        overviewLead: "Grounded in your Git repositories and development standards, it understands the project context and answers accurately. Source stays protected in an on-premise environment, and an open architecture with no lock-in to a single AI model leaves you free to choose the LLM that fits your organization.",
        valueTitle: "Development productivity and operational stability, together",
        valueLead: "Project-specific code comprehension and safe on-premise operation raise development speed while cutting maintenance cost.",
        labelBefore: "Before",
        labelAfter: "After",
        labelValue: "Value",
        capabilitiesTitle: "Core capabilities",
        capabilitiesLead: "Capabilities built on learning your code, APIs, and schemas, so answers land in your project's context.",
        howTitle: "How it works",
        howLead: "It indexes the repository into evidence, retrieves the context the question needs, and generates a code-level answer.",
        archNote: "The full structure — gateway-based authentication isolation plus indexing, RAG, and MCP — is documented in the architecture pages.",
        archCta: "Read the Code Assistant architecture",
        comparisonTitle: "How this differs from a general coding assistant",
        comparisonGeneric: "General coding assistant",
        integrationsTitle: "Integration and deployment",
        ...
        ctaTitle: "Let's evaluate it against your codebase",
        ctaLead: "We design the rollout together, fitted to your repositories and development environment.",
        ctaPrimary: "Talk to us",
        ctaSecondary: "Discuss security requirements",
    },
};

export function CodeAssistantPageContent({ locale }: { locale: Locale }) {
    const t = COPY[locale];
    const home = locale === "en" ? "/en" : "/";
    const CA_SECTIONS = t.sections;
    const SPECS = t.specs.map(([label, detail], i) => ({ icon: SPEC_ICONS[i], label, detail }));
    const OUTCOMES = t.outcomes.map((o, i) => ({ ...o, icon: OUTCOME_ICONS[i] }));
    const TRUST = t.trust.map(([label, sub], i) => ({ icon: TRUST_ICONS[i], label, sub }));
    const PILLARS = t.pillars.map(([title, desc], i) => ({ icon: PILLAR_ICONS[i], title, desc }));
    const CAPABILITIES = t.capabilities.map((c, i) => ({ ...c, icon: CAPABILITY_ICONS[i] }));
    const STEPS = t.steps.map((st, i) => ({ ...st, step: String(i + 1).padStart(2, "0") }));
    const INTEGRATIONS = t.integrations.map(([title, desc], i) => ({ icon: INTEGRATION_ICONS[i], title, desc }));
    const COMPARISON = t.comparison.map(([aspect, generic, ca]) => ({ aspect, generic, ca }));
    const USE_CASES = t.useCases.map((u, i) => ({ ...u, icon: USE_CASE_ICONS[i] }));
    const FAQ = t.faq;
    return (
        <>
            <SiteNav overlay />
                ...
                data={[
                    {
                        ...
                        inLanguage: locale,
                        url: absoluteUrl(localeHref(locale, "/code-assistant")),
                ...
                <div className="relative mx-auto grid w-full max-w-7xl gap-10 px-6 pt-16 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
                    <div>
                        ...
                        <div className="mt-8 flex flex-wrap gap-3">
                            <Link
                                href={localeHref(locale, "/contact") + "?from=code-assistant"}
                            ...
                            <Link
                                href={localeHref(locale, "/customers") + "?product=code-assistant"}
                    ...
                    <CodeAssistantHeroArt locale={locale} />
                </div>
            </section>
                ...
                <section id="how-it-works" className="scroll-mt-[calc(var(--nav-h,84px)+58px)] border-t border-[var(--color-line)] bg-[var(--color-surface)]">
                    <div className="mx-auto max-w-7xl px-6 py-24">
                        ...
                        <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface-alt)] p-6 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <Link
                                href={localeHref(locale, "/architecture") + "#code-assistant"}
                ...
                <section className="border-t border-[var(--color-line)] bg-[var(--color-surface-alt)]">
                    <div className="mx-auto flex max-w-7xl flex-col items-start gap-4 px-6 py-16 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex shrink-0 flex-wrap gap-3">
                            ...
                            <Link
                                href={localeHref(locale, "/contact") + "?type=poc&from=code-assistant"}
