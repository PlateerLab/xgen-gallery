...
/** 글 상세 화면 문구. 본문·제목은 content/blog(/en) 의 원문을 그대로 쓴다. */
const COPY: Record<
    Locale,
    {
        writtenBy: string;
        ...
        seriesNote: (idx: number, total: number) => string;
        summary: string;
        faqAria: string;
        faqTitle: string;
        seriesNavAria: string;
        prevPart: string;
        nextPart: string;
        backToList: string;
    }
> = {
    ko: {
        writtenBy: "작성",
        editedBy: "편집",
        coverAlt: (title) => `${title} — 커버 일러스트`,
        seriesNote: (idx, total) => `시리즈 ${idx}번째 글 · 전체 ${total}편 보기`,
        summary: "핵심 요약",
        faqAria: "자주 묻는 질문",
        faqTitle: "자주 묻는 질문",
        seriesNavAria: "시리즈 이동",
        prevPart: "이전 편",
        nextPart: "다음 편",
        backToList: "블로그 목록으로",
    },
    en: {
        ...
        editedBy: "Edited by",
        coverAlt: (title) => `${title} — cover illustration`,
        seriesNote: (idx, total) => `Part ${idx} of ${total} — see the whole series`,
        summary: "In short",
...
export async function BlogPostPageContent({
    ...
    const sposts = sdef ? seriesPosts(sdef, getAllPosts(locale)) : [];
                ...
                <div className="relative mx-auto w-full max-w-3xl px-6 pt-16">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[14px] text-white/60">
                        <span className="inline-flex items-center gap-1.5">
                            {t.writtenBy} <span className="text-white/40">|</span>{" "}
                            {post.authorGithub ? (
                                <Link
                                    href={localeHref(locale, `/members/${post.authorGithub}`)}
                        ...
                        {post.editor && (
                            <span>
                                {t.editedBy} <span className="text-white/40">|</span>{" "}
            ...
            <main className="mx-auto max-w-3xl px-6 py-20">
                ...
                {sdef && (sPrev || sNext) && (
                    ...
                    >
                        {sPrev ? (
                            <Link
                                href={localeHref(locale, `/blog/${sPrev.slug}`)}
                        ...
                        {sNext && (
                            <Link
                                href={localeHref(locale, `/blog/${sNext.slug}`)}
