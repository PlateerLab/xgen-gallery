...
export function FeaturedCaseCard({
            ...
            <div className="relative aspect-[16/10] overflow-hidden">
                ...
                <div className="absolute left-4 top-4 flex flex-wrap items-center gap-2 text-[12.5px]">
                    ...
                    <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-white/85 backdrop-blur-sm">
                        {en ? INDUSTRIES[item.industry].en : INDUSTRIES[item.industry].ko}
                    </span>
                    <time
    ...
    return (
        <Link href={localeHref(locale, `/customers/case/${item.slug}`)} className={cls}>
            {body}
        </Link>
    );
}
