 ...
 *
 * 이 사례만 일반 사례 템플릿(/customers/case/[slug]의 기본 레이아웃) 대신 전용
 * 레이아웃을 쓴다. 공개 레퍼런스라 서사를 길게 실을 수 있는 유일한 사례이기 때문.
 * 메타데이터·JSON-LD·사이트맵은 [slug] 라우트가 lib/customers.ts를 보고 공통으로
 * 생성하므로, 여기서는 본문만 담당한다.
 *
 * 대외비(계약 금액·수행 인력·내부 일정 등)는 담지 않는다 — lib/customers.ts 상단
 * 운영 원칙 참조.
 */
