...
import { SERIES, seriesPosts } from "@/lib/series";
...
export async function generateMetadata({
    ...
    return {
        ...
        alternates: { canonical: `/blog/series/${def.key}` },
        openGraph: {
...
export default async function SeriesPage({
    ...
    const def = SERIES.find((s) => s.key === key);
    if (!def) notFound();
    const posts = seriesPosts(def, getAllPosts());
            ...
            <JsonLd
                data={[
                    breadcrumbLd([
                        { name: "Home", path: "/" },
                        { name: "Blog", path: "/blog" },
                        { name: def.title, path: `/blog/series/${def.key}` },
                    ]),
                    itemListLd(
                        def.title,
                        posts.map((p) => ({
