...
const vol4: Issue = {
    ...
    inProgress: [
        ...
        {
            title: "Geny, 자기확장을 격리 위에 올렸습니다",
            badge: "개발중",
            percent: 70,
            body: "에이전트가 만든 도구가 남는 이야기를 지난 호에 전해드렸는데, 이번 2주는 그 도구가 자라는 환경을 만들었습니다. 사용자마다 격리된 작업자 파드가 떠서 남의 것을 읽을 수 없고, 세션이 끝나면 환경이 통째로 보존됐다가 다음 세션에 되살아납니다. 등록 전 검증 게이트가 생겨 시험을 통과한 도구만 목록에 오릅니다. 8월 28일 운영 배포에는 Geny 노드를 일부러 끄고 내보냈습니다. 격리가 검증을 마칠 때까지 운영에서는 잠시 만날 수 없습니다.",
        },
        {
            title: "사용자 클라우드 스토리지",
            badge: "개발중",
    ...
    upcoming: [
        {
            ...
            body: "만들며 배운 것을 글로 남기고 있습니다. Geny를 소개하는 \"답변하는 AI에서, 스스로 일하는 AI Agent로\", 분산 추적을 들이며 겪은 일을 적은 \"XGEN을 위한 Jaeger 도입기\", 그리고 온톨로지 카드에서 언급한 \"LLM 없이 온톨로지 그래프를 빌드해봤습니다\"까지 세 편이 올라갔습니다. labs.plateer.com 블로그에서 읽으실 수 있습니다.",
...
const vol3: Issue = {
    ...
    inProgress: [
        {
            title: "Geny — 에이전트가 만든 도구가 남습니다",
            badge: "개발중",
            percent: 60,
            body: "에이전트가 작업 중에 만들어낸 도구를 다음 세션에도 쓸 수 있게 하고 있습니다. 도구 스펙과 실행 환경을 DB에 두어 파드가 다시 떠도 사라지지 않고, 확정된 의존성 버전을 스펙에 함께 적습니다. 코드 실행은 별도 러너로 옮겨 워크플로우 본체와 분리했고, [도구] 뷰에서 실제 코드와 3단계 상태를 보며 그 자리에서 테스트를 돌립니다. 사용자에게 보이는 제품명은 Geny로 통일합니다(내부 식별자는 그대로입니다).",
        },
        {
            title: "음성 파일 전사",
            badge: "개발중",
    ...
    papers: [
        {
            ...
            body: "에이전트가 매 요청마다 같은 절차의 코드를 다시 만드는 낭비를 다룬 논문입니다(Kalle Kujanpää 외, 7월 9일). 반복되는 업무 절차를 미리 검증하고 버전을 매긴 도구로 컴파일해두는 방식으로 바꾸자, 실제 서비스에서 지연이 최대 42% 줄고 오류율이 53% 개선됐습니다. 무엇이 언제 쓰였는지 추적하기도 쉬워졌습니다. 이번 호 Geny가 서 있는 자리와 같습니다.",
